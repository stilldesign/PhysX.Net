...
void Cloth::SetCollisionSpheres(array<ClothCollisionSphere>^ spheres)
{
	...
	this->UnmanagedPointer->setCollisionSpheres((PxClothCollisionSphere*)pin);
}
...
void Cloth::SetCollisionPlanes(array<ClothCollisionPlane>^ planes)
{
	...
	// TODO: 0 length array error
	pin_ptr<ClothCollisionPlane> p = &planes[0];

	this->UnmanagedPointer->setCollisionPlanes((PxClothCollisionPlane*)p);
}
...
ClothPhaseSolverConfig^ Cloth::GetPhaseSolverConfig(ClothFabricPhaseType phaseType)
{
	PxClothPhaseSolverConfig config = this->UnmanagedPointer->getPhaseSolverConfig(ToUnmanagedEnum(PxClothFabricPhaseType, phaseType));

	return ClothPhaseSolverConfig::ToManaged(config);
}
void Cloth::SetPhaseSolverConfig(ClothFabricPhaseType phaseType, ClothPhaseSolverConfig^ config)
{
	PxClothPhaseSolverConfig c = ClothPhaseSolverConfig::ToUnmanaged(config);

	this->UnmanagedPointer->setPhaseSolverConfig(ToUnmanagedEnum(PxClothFabricPhaseType, phaseType), c);
}
...
void Cloth::WakeUp(float wakeCounter)
{
	this->UnmanagedPointer->wakeUp(wakeCounter);
}
...
ClothReadData^ Cloth::LockClothReadData()
{
	return gcnew ClothReadData(this->UnmanagedPointer->lockClothReadData(), this->UnmanagedPointer->getNbParticles());
}
...
PhysX::MotionConstraintScaleBias Cloth::MotionConstraintScaleBias::get()
{
	float s, b;
	this->UnmanagedPointer->getMotionConstraintScaleBias(s, b);

	PhysX::MotionConstraintScaleBias scaleBias;
		scaleBias.Scale = s;
		scaleBias.Bias = b;

	return scaleBias;
}
void Cloth::MotionConstraintScaleBias::set(PhysX::MotionConstraintScaleBias value)
{
	float s = value.Scale;
	float b = value.Bias;

	this->UnmanagedPointer->setMotionConstraintScaleBias(s, b);
}
...
float Cloth::InertiaScale::get()
{
	return this->UnmanagedPointer->getInertiaScale();
}
void Cloth::InertiaScale::set(float value)
{
	this->UnmanagedPointer->setInertiaScale(value);
}
...
float Cloth::DragCoefficient::get()
{
	return this->UnmanagedPointer->getDragCoefficient();
}
void Cloth::DragCoefficient::set(float value)
{
	this->UnmanagedPointer->setDragCoefficient(value);
}
