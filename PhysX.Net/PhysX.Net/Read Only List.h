...
namespace StillDesign
{
	namespace PhysX
	{
		...
		public ref class ReadOnlyList : System::Collections::ObjectModel::ReadOnlyCollection<T>
		{
			...
			public:
				ReadOnlyList() : System::Collections::ObjectModel::ReadOnlyCollection<T>( gcnew InternalList<T>() )
				{
					InternalList<T>^ list = (InternalList<T>^)this->Items;
					
					list->onAdd += gcnew EventHandlerItem<T>( this, &ReadOnlyList::list_onAdd );
					list->onRemove += gcnew EventHandlerItem<T>( this, &ReadOnlyList::list_onRemove );
				}
				ReadOnlyList( ListBase<T>^ list ) : System::Collections::ObjectModel::ReadOnlyCollection<T>( list )
				{
					list->onAdd += gcnew EventHandlerItem<T>( this, &ReadOnlyList::list_onAdd );
					list->onRemove += gcnew EventHandlerItem<T>( this, &ReadOnlyList::list_onRemove );
				}
			...
			private:
				void list_onAdd( Object^ sender, T item )
				{
					RaiseAdd( item );
				}
				void list_onRemove( Object^ sender, T item )
				{
					RaiseRemove( item );
				}
			...
			public:
				...
				array<T>^ ToArray()
				{
					return (dynamic_cast<ListBase<T>^>( this->Items ))->ToArray();
				}
