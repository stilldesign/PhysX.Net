...
namespace StillDesign
{
	public partial class Demo
	{
		private void LoadPhysics()
		{
			...
			//var p = StillDesign.PhysX.Utilities.Utilities.LoadCollection( "Scene 1.dae", StillDesign.PhysX.Utilities.UtilitiesFileType.Collada );

			//p.InstantiateCollection( _core, _scene, null, new LoadUserNotify() );
			...
			for( int x = 0; x < 20; x++ )
			{
				...
				ActorDescription actorDesc = new ActorDescription();
					...
					actorDesc.BodyDescription = new BodyDescription( 10.0f );
					actorDesc.GlobalPose = Matrix.CreateTranslation( 0, 15 + 3 * x, 20 );
					actorDesc.Shapes.Add( boxShapeDesc );
			...
			#region Cloth (Flag)
			{
				...
				ClothDescription clothDesc = new ClothDescription();
					...
					clothDesc.GlobalPose =
						Matrix.CreateFromYawPitchRoll( 0, 1.5708f, 1.5708f ) *
